function  [ObsPointlist] = C130ObservationPointGeneration(ACLocInMeter)
    % C-130 LOCATION
...
for ACnum = 1 : length(C130BUNO)
    ...
    title([{' Top View','C-130(s), Observation Points & Inspection Drones'}],'Fontsize',14)
    ...
    CameraAzimuth = [15, 30, 45, 60, 75, 90, 105, 145, 275, 345, 15, 30, 45, 60, 75, 90, 105, 145, 275, 345, ...
        15, 30, 45, 60, 75, 90, 105, 145, 275, 345, 15, 30, 45, 60, 75, 90, 105, 145, 275, 345, ...
        15, 30, 45, 60, 75, 90, 105, 145, 275, 345, 15, 30, 45, 60, 75, 90, 105, 145, 275, 345, ...
        15, 30, 45, 60, 75, 90, 105, 145, 275, 345, 15, 30, 45, 60, 75, 90, 105, 145, 275, 345] + azimuth;
    CameraAngle = [0, 20, 40, 60, 80, 100, 120, 140, 160, 180, 0, 20, 40, 60, 80, 100, 120, 140, 160, 180, ...
    ...
    L = length(ObservWPx);
    D = D + L;
    ObsPointlist(C:D,1) = C130BUNO(1, ACnum); % Aircraft Observation Point belongs too
    ObsPointlist(C:D,2) = ObservWPx';       % Observation point Longitudes
    ObsPointlist(C:D,3) = ObservWPy';       % Observation point Lattitudes
    ObsPointlist(C:D,4) = ObservWPz';       % Observation point Heights
    ObsPointlist(C:D,5) = CameraAzimuth';   % Camera azimuth at Observation Point
    ObsPointlist(C:D,6) = CameraAngle';     % Camera angle at Observation Point
    C = C + L;
end